...
contract Quarters is Ownable, ERC20 {
  ...
  function updatePlayerRewards(address _address) internal {
    ...
    if (_reward > 0) {
      ...
      _balances[_address] += _reward;
      _allowances[_address][msg.sender] += _reward; // set allowance

      _totalSupply += _reward;
      ...
      emit Reward(_address, _reward, outstandingQuarters, _totalSupply);
    }
  }
  ...
  function burn(uint256 _value) public returns (bool success) {
    ...
    // log rate change
    emit BaseRateChanged(getBaseRate(), tranche, outstandingQuarters, address(this).balance, _totalSupply);
    return true;
  }
  ...
  IMigrationTarget(migrationTarget).migrateFrom(msg.sender, _amount, rewards[msg.sender], trueBuy[msg.sender], developers[msg.sender]);
   }
